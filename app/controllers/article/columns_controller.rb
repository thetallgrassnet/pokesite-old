class Article::ColumnsController < ApplicationController
  def index
    ...
    @posts = Article::Post.as(:p).published(:p).with_associations(:author, :column).page(params[:page]).per(10)
  ...
  def show
    ...
    @posts = @column.posts(:p).published(:p).with_associations(:author, :column).page(params[:page]).per(10)
    (not_found and return) if params[:page] and @posts.count == 0
